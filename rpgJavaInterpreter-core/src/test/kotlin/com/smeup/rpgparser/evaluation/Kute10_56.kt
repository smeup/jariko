...
class Kute10_56 {
    ...
    public fun performanceComparing() {
        ...
        val actualElapsedTimeInMillisec = measureTimeMillis {
            // 'DOU' implementation
            doUntil(equalityExpr, evalStatement)
        }
    ...
    private fun doUntil(equalityExpr: Expression, evalStatement: EvalStmt) {
        do {
        ...
        return when (expression) {
            ...
            is DataRefExpr -> get(
                    expression.variable.referred
                            ?: throw IllegalStateException("[Kute10_56] Unsolved reference ${expression.variable.name} at ${expression.position}")
            )
            is EqualityExpr -> {
