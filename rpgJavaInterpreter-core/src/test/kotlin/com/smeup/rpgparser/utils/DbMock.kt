...
interface DbMock : AutoCloseable {
    ...
    fun populateTable(values: List<Map<String, Any>>): String =
        if (values.isEmpty()) buildDefaultPopulationQuery() else buildPopulationQuery(values)
    ...
    fun buildDbColumnsFromDbFields(fields: List<DbField>): String =
        fields
            .mapIndexed { i, it ->
                when (it.type) {
                    is StringType ->
                        "\"${it.fieldName}\" VARCHAR(${it.type.size}) DEFAULT '' NOT NULL".plus(
                            if (fields.lastIndex !=
                                i
                            ) {
                                ",\n"
    ...
    private fun madeInsertStatement(
        names: String,
        values: String,
    ): String =
        """
    ...
    private fun List<Map<String, Any>>.getFromValues(values: List<DbField>): String {
        val result =
            this.joinToString(DB_ITEM_SEPARATOR) { value ->
                val formattedValue =
                    values.joinToString(DB_ITEM_SEPARATOR) formatter@{ field ->
