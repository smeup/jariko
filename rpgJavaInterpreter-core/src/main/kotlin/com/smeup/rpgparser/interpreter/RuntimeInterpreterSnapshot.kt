package com.smeup.rpgparser.interpreter

/**
 * An instance of this object will be used by the client to inform jariko about
 * interpreter state just before the EXFMT statement execution.
 */
data class RuntimeInterpreterSnapshot(val id: String) {
    companion object {
        fun blank(): RuntimeInterpreterSnapshot {
            return RuntimeInterpreterSnapshot("")
        }

        fun from(interpreterCore: InterpreterCore): RuntimeInterpreterSnapshot {
            return RuntimeInterpreterSnapshot("from-interpreter")
        }
    }
}