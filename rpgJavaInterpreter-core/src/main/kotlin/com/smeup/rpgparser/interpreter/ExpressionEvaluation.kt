...
class ExpressionEvaluation(
    ...
    private fun evalAsInt(expression: Expression): Int =
        expression
            .evalWith(this)
            .asInt()
            .value
    ...
    private inline fun proxyLogging(
    ...
    override fun eval(expression: UDateRefExpr) =
        proxyLogging(expression) {
            val formatter = DateTimeFormatter.ofPattern("ddMMyy")
            val date = LocalDate.now().format(formatter)
            StringValue(date)
        }

    override fun eval(expression: UYearRefExpr) =
        proxyLogging(expression) {
    ...
    override fun eval(expression: UMonthRefExpr) =
        proxyLogging(expression) {
    ...
    override fun eval(expression: UDayRefExpr) =
        proxyLogging(expression) {
    ...
    override fun eval(expression: DecNumericExpr): Value =
        proxyLogging(expression) {
            val decDigits =
                expression.decDigits
                    .evalWith(this)
                    .asInt()
                    .value
            val valueAsString =
            ...
            valueAsBigDecimal ?: throw ProgramStatusCode.INVALID_CHARACTERS.toThrowable(
                additionalInformation = "value: $valueAsString",
    ...
    override fun eval(expression: DecTimeExpr): Value =
        proxyLogging(expression) {
    ...
    override fun eval(expression: LookupGtExpr): Value =
        proxyLogging(expression) {
            lookup(
                array = expression.array.evalWith(this) as ArrayValue,
                arrayType = expression.array.type() as ArrayType,
    ...
    override fun eval(expression: LookupGeExpr): Value =
        proxyLogging(expression) {
            lookup(
                array = expression.array.evalWith(this) as ArrayValue,
                arrayType = expression.array.type() as ArrayType,
    ...
    override fun eval(expression: LookupLtExpr): Value =
        proxyLogging(expression) {
            lookup(
                array = expression.array.evalWith(this) as ArrayValue,
                arrayType = expression.array.type() as ArrayType,
    ...
    override fun eval(expression: LookupLeExpr): Value =
        proxyLogging(expression) {
            lookup(
                array = expression.array.evalWith(this) as ArrayValue,
                arrayType = expression.array.type() as ArrayType,
    ...
    override fun eval(expression: AllExpr) =
        proxyLogging(expression) {
    ...
    override fun eval(expression: NotExpr) =
        proxyLogging(expression) {
    ...
    override fun eval(expression: ScanExpr): Value =
        proxyLogging(expression) {
            ...
            val source =
                expression.length?.evalWith(this)?.asInt()?.value?.toInt()?.let { length ->
                    expression.source
                        .evalWith(this)
                        .asString()
                        .value
                        .substring(0, startIndex + length)
                } ?: expression.source
                    .evalWith(this)
                    .asString()
                    .value
    ...
    override fun eval(expression: SubarrExpr): Value =
        proxyLogging(expression) {
            ...
            val numberOfElement: Int? =
                if (expression.numberOfElements !=
                    null
                ) {
                    expression.numberOfElements
                        .evalWith(this)
                        .asInt()
                        .value
                        .toInt()
    ...
    override fun eval(expression: FunctionCall): Value =
        proxyLogging(expression) {
            ...
            callback.traceBlock(trace) {
                ...
                val function =
                    systemInterface.findFunction(interpreterStatus.symbolTable, functionToCall)
    ...
    override fun eval(expression: EditcExpr): Value =
        proxyLogging(expression) {
            ...
            if (format !is StringValue) {
                throw UnsupportedOperationException(
                    "Required string value, but got $format at ${expression.position}",
                )
            }
                ...
                is DurationInMonths ->
                    IntValue(
                        ChronoUnit.MONTHS.between(
                            v2.asTimeStamp().localDate,
                            v1.asTimeStamp().localDate,
                        ),
                    )
                is DurationInYears ->
                    IntValue(
    ...
    override fun eval(expression: DivExpr): Value =
        proxyLogging(expression) {
            ...
            if (v2.bigDecimal.isZero()) {
                throw ProgramStatusCode.DIVIDE_BY_ZERO.toThrowable(
                    additionalInformation = "v1: $v1, v2: $v2",
                    position = expression.position,
                )
            }
    ...
    override fun eval(expression: TrimExpr) =
        proxyLogging(expression) {
    ...
    override fun eval(expression: QualifiedAccessExpr): Value =
        proxyLogging(expression) {
            val dataStringValue =
                when (val value = expression.container.evalWith(this)) {
                    is DataStructValue -> {
                        value
                    }
            ...
            return@proxyLogging dataStringValue[
                expression.field.referred
    ...
    override fun eval(expression: SqrtExpr): Value =
        proxyLogging(expression) {
            ...
            if (sqrt.isNaN()) {
                ProgramStatusCode.NEGATIVE_SQUARE_ROOT.toThrowable(
            ...
            for (i in IndicatorType.Predefined.range) {
                value.append(
                    if (interpreterStatus.indicators[i] == null) {
                        "0"
                    } else if (interpreterStatus.indicators[i]!!.value) {
                        "1"
                    } else {
                        "0"
                    },
    ...
    override fun eval(expression: OpenExpr): Value =
        proxyLogging(expression) {
            ...
            val enrichedDBFile =
                interpreterStatus.dbFileMap.get(name)
    ...
    override fun eval(expression: SizeExpr): Value =
        proxyLogging(expression) {
            return@proxyLogging when (expression.value) {
                is DataRefExpr ->
                    (expression.value as DataRefExpr)
                        .variable.referred
                        ?.type
                        ?.size
                        ?.let { IntValue(it.toLong()) }
                ...
                //  - multiple-occurrence data structure{:*ALL}
                    ...
                    "I do not know how to handle ${expression.value} with %SIZE. Is '${expression.value.javaClass.simpleName}' class.",
                )
            }
        }
    ...
    override fun eval(expression: XFootExpr): Value =
        proxyLogging(expression) {
    ...
    override fun eval(expression: ReallocExpr): Value =
        proxyLogging(expression) {
    ...
    private fun lookupLinearSearch(
        values: List<Value>,
        target: Value,
    ): Int {
    ...
    private inline fun lookupBinarySearchWithCondition(
        values: List<Value>,
        predicate: (Value) -> Boolean,
        target: Value,
    ): Int {
    ...
    private fun lookup(
        ...
        val index =
            if (isSequenced) {
                ...
                lookupLinearSearch(searchRange, searchedValue)
            }
                ...
                left.elementType is StringType && right.elementType is StringType ->
                    left.elements().mapIndexed { i: Int, v: Value ->
                        if (right.elements().size > i) {
                            ...
                            val valueToAdd: String =
                                if (v.asString().varying) {
                                    (v.asString().value + rightElement.asString().value)
                                } else {
                                    (
                                        v.asString().value + " ".repeat(left.elementType.elementSize() - v.asString().value.length) +
                                            rightElement.asString().value
                                    )
                                }
                        ...
                        if (right.elements().size > i) {
                            ...
                            when {
                                ...
                                v is NumberValue && rightElement is NumberValue -> {
                                    ...
                                    "Unable to sum ${left.elementType} and ${right.elementType} as Array elements at $position",
                                )
                            }
                        } else {
                            v
