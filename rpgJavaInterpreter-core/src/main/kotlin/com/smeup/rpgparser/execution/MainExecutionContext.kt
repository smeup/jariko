...
class LoggingContext {
    ...
    fun generateTimeUsageByStatementReportEntries(source: LogSourceData): List<LazyLogEntry> {
        return timeUsageByStatement.toList().map {
            ...
            LazyLogEntry(entry) { sep ->
                "$statementName$sep${duration.toString(DurationUnit.MICROSECONDS)}$sep$timePercentage${sep}$hit${sep}avg. ${average.toString(DurationUnit.MICROSECONDS)}"
            }
        }
    }
    ...
    fun generateSymbolTableTimeUsageReportEntries(source: LogSourceData): List<LazyLogEntry> {
        return symbolTableTimeUsage.toList().map {
            ...
            LazyLogEntry(entry) { sep ->
                "${action.name}$sep${duration.toString(DurationUnit.MICROSECONDS)}$sep$timePercentage${sep}$hit${sep}avg. ${average.toString(DurationUnit.MICROSECONDS)}"
            }
        }
    }
    ...
    fun generateExpressionReportEntry(source: LogSourceData): LazyLogEntry {
        ...
        return LazyLogEntry(entry) { sep ->
            "${duration.toString(DurationUnit.MICROSECONDS)}$sep$timePercentage$sep$hit${sep}avg. ${average.toString(DurationUnit.MICROSECONDS)}"
        }
    }
