...
class ResourceProgramFinder(val path: String) : RpgProgramFinder {
    override fun findRpgProgram(nameOrSource: String): RpgProgram? {
        ...
        return if (resourceStream != null) {
            RpgProgram.fromInputStream(resourceStream, nameOrSource)
        } else {
            null
        }
    }
...
object RunnerCLI : CliktCommand() {
    ...
    private val compiledProgramsDir by option("-cpd", "--compiled-programs-dir").file(mustExist = true, mustBeReadable = true)
    private val programArgs by argument().multiple(required = false)
    private val reloadConfigurationFile by option("-rc", "--reload-config").file(mustExist = true, mustBeReadable = true)
