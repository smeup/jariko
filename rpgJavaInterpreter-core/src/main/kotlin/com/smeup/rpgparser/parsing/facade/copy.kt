                ...
                .runCatching {
                    ...
                    val copy =
                        (findCopy.invoke(copyId))
                            ...
                            ?.surroundWithPreprocessingAnnotations(copyId)
                            ?.apply {
                            } ?: let {
...
class CopyBlocks : Iterable<CopyBlock> {
     ...
     */
    override fun iterator(): Iterator<CopyBlock> = copyBlocks.iterator()

    operator fun get(index: Int) = copyBlocks[index]

    internal fun onStartCopyBlock(
        copyId: CopyId,
        start: Int,
    ) {
    ...
    internal fun getCopyBlock(absoluteLine: Int): CopyBlock? =
        copyBlocks
            .filter { copyBlock ->
                absoluteLine >= copyBlock.start && absoluteLine < copyBlock.end
    ...
    fun relativeLine(absoluteLine: Int): RelativeLine {
        ...
        val relativeLine =
            copyBlock?.let { block ->
                ...
                absoluteLine - (
                    getCopyBlocksBefore(absoluteLine)
