...
| Operation Code | Support level | Rationale | Example |
| ------ | ------ | ------ | ------ |
...
| CALL | Partial | Error indicator, parameter passing, and `callProgramHandler` all handled; `*OMIT` parameters not supported | `C     CALL      'MYPGM'`<br>`C     PARM                    *OMIT` — `*OMIT` parameter not supported |
...
| CLOSE | Partial | Sets `dbFile.open = false` for DB files; printer files have no corresponding dbFile entry | `C     CLOSE     MYRPT` (MYRPT declared as PRINTER) — printer file close is a no-op |
...
| DSPLY | Partial | Sends output to `systemInterface.display()`; response-input `TODO` not implemented | `C     'Enter Y/N'   DSPLY                   resp` — response-input result field hits `TODO` |
...
| EVAL | Partial | All assignment operators covered; `EVAL(M)` and `EVAL(R)` extenders hit `TODO` inside `DivExpr` | `C     EVAL(M)   result = a / b` — `EVAL(M)` extender hits `TODO` in `DivExpr` |
| EVALR | Partial | Only `StringType` and `DataStructureType` targets allowed; throws for numeric or other result fields | `D n   S              5P 0`<br>`C     EVALR     n = *BLANKS` — numeric target field throws |
...
| GOTO | Partial | Throws `GotoException`; handled at subroutine and top-level loops; GOTO into nested structures may misbehave | `C     GOTO      INNER`<br>`C     IF        *IN01`<br>`C     INNER     TAG`<br>`C     ENDIF` — GOTO into nested IF body may misbehave |
...
| KFLD | Partial | Fields stored within `KListStmt`; actual key-type coercion delegated to `accessFieldsType` metadata | `C     KL1           KLIST`<br>`C     KFLD                    numKey` (char-keyed file) — key-type coercion via `accessFieldsType` metadata may fail |
| KLIST | Partial | Stores field list in `status.klists`; used by CHAIN/SETLL/SETGT via `toSearchValues()` | `C     KL1           KLIST`<br>`C     KFLD                    keyFld`<br>`C     KL1           CHAIN     MYFILE` — `toSearchValues()` may fail if `accessFieldsType` metadata is incomplete |
...
| MOVE | Partial | Delegates to `move()` helper; type conversion may not cover all source/target type combinations | `D d   S               D   INZ(D'2024-01-15')`<br>`C     MOVE      d             charFld` — date-to-char conversion not confirmed for all type pairings |
| MOVEA | Full | Delegates to `movea()` helper; handles array-to-field and field-to-array moves | - |
| MOVEL | Partial | Delegates to `movel()` helper; left-aligned move; not all edge cases confirmed tested | `D t   S             10A`<br>`D s   S              5A   INZ('Hello')`<br>`C     MOVEL(P)  s             t` — edge cases with `(P)` extender and type combinations unconfirmed |
| MULT | Full | Supports optional Factor 1 and half-adjust extender; delegates to `mult()` helper | - |
| MVR | Partial | Execute is a no-op; remainder is stored to target by `DivStmt` during DIV execution | `C     MVR                     rem` — execute is a no-op; valid only immediately after DIV |
| OCCUR | Full | Sets or gets occurrence index of an `OccurableDataStructValue`; error indicator for out-of-range | - |
| OPEN | Partial | Sets `dbFile.open = true` for DB files; printer files only log a warning and return | `C     OPEN      MYRPT` (MYRPT declared as PRINTER) — printer file open only logs a warning; `dbFile.open` not set |
...
| RESET | Partial | Restores `DataDefinition` to its `defaultValue`; `*ALL` operand and indicator targets not handled | `C     RESET     *ALL` — `*ALL` operand not handled; throws |
| RETURN | Partial | Sets RT indicator and throws `ReturnException`; LR indicator semantics not fully implemented | `C     SETON                                 LR`<br>`C     RETURN` — LR-mode program deinitialization incomplete; subsequent call may not reinitialize |
...
| SORTA | Partial | Delegates to `sortA()` helper; keyed-DS-array sort may not be fully supported | `D myDS DS                 QUALIFIED DIM(5)`<br>`C     SORTA(A:keyFld)myDS` — keyed-DS-array sort not fully supported |
...
| SUBDUR | Partial | Delegates to `DiffExpr`; limited to cases where `asTimeStamp()` succeeds on both operands | `D d1  S               D   INZ(D'2024-06-01')`<br>`C     d1            SUBDUR    d2            result:*DAYS` — `DateValue` operand may fail on `asTimeStamp()` |
...
| TIME | Partial | Handles `TimeStampType` and 6/12/14-digit `NumberType`; character result field not supported | `D timeStr S              6A`<br>`C     TIME                    timeStr` — character result field not supported |
